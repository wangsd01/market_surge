...
**Anchor**: if a `BreakoutEvent` exists and is within `anchor_max_age_bdays`, anchor = index of the highest `High` from the breakout bar onward -- *unless* that index is the latest bar itself (no room to scan forward from it), in which case fall through to the lookback fallback below instead of giving up. This matters in practice: a currently-still-rising stock will almost always have *today's* bar independently qualify as "the breakout" (a fresh high plus a decent body alone gets most of the way to `breakout_score_min_threshold`), and without the fallthrough, `compute_h1h2_state` would return `None` for exactly the stocks where `EXTENDED_AFTER_H2` matters most -- actively-extending ones.

Otherwise (or on that fallthrough), anchor = the **start** of the `anchor_lookback_bars` window (`max(0, latest_idx - anchor_lookback_bars)`), not any single "best" swing point picked by `argmax`/`argmin` of High or Low. `scan_from_anchor`'s own `seeking_pullback` state already walks forward through any number of rising bars undisturbed until the first genuine pullback appears, so starting earlier than strictly necessary never changes the outcome for a clean uptrend -- but a *global* max/min within the window can land *after* the interesting structure (a later bar that's even higher than the true swing high, or a failure that makes an even lower low than the structure being anchored for), silently skipping exactly the H1/H2 sequence the anchor exists to find. `None` if the window collapses to a single bar.
