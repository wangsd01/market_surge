...
def run(args: argparse.Namespace):
    ...
    filtered = _filter_screened_summary(artifacts.summary, args)
    _save_screened_ranking(filtered, run_dir)
    ...
    _save_ticket_charts(all_valid_tickets if args.debug else tickets, artifacts.raw_df, run_dir)
    return tickets
...
def _save_screened_ranking(filtered: pd.DataFrame, run_dir: Path) -> None:
    if filtered.empty:
        return
    filtered.to_csv(run_dir / "screened_ranking.csv", index=False)


def _save_ticket_charts(tickets, raw_df: pd.DataFrame, run_dir: Path) -> list[Path]:
    if not tickets:
        return []
